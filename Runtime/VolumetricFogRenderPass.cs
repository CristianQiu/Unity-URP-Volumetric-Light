...
public sealed class VolumetricFogRenderPass : ScriptableRenderPass
{
	...
	{
		// TODO: Forward+ and deferred+ visibleLights.Length is 256. In forward, it is 257 so the main light is considered apart.
		// In deferred it seems to not have any limit (seen 1.6k and beyond). All rendering paths
		// have maxVisibleAdditionalLights at 256. For the time being, just assume that if
		// additional lights contribution to fog is enabled, we are either using clustered
		// rendering, or using forward or deferred with, at most, 257 visible lights.
